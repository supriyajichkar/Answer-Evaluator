...
def availableTestView(request):
    objects = list(models.Test.objects.filter(
        section__section_name=request.session['section']))
    availableObjects = []
    yet = []
    for i in objects:
        if i.end_time > timezone.now():
            ...
            if timezone.now() < i.start_time:
                yet.append(i)
    print(yet)
    return render(request, 'available.html', context={'objects': availableObjects, 'yet': yet})
...
def evaluateView(request):
    ...
    for i in indexes:
        ...
        if givenAnswer:
            ...
            print(sentences)
            similarity_input_placeholder = tf.placeholder(
                tf.string, shape=(None))
            similarity_message_encodings = embed(similarity_input_placeholder)
            with tf.Session() as session:
                session.run(tf.global_variables_initializer())
                session.run(tf.tables_initializer())
                message_embeddings_ = session.run(similarity_message_encodings, feed_dict={
    ...
    result = models.Result.objects.create(
        user=request.user, test=test, marks=sum(marksF), section=test.section)
    result.save()
